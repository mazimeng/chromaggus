...
import com.workasintended.chromaggus.component.{CharacterComponent, DeadComponent, JobComponent, MovementComponent}
...
class Blackboard {
  ...
  val enemyFamily: Family = Family.all(classOf[CharacterComponent]).exclude(classOf[DeadComponent]).get()
